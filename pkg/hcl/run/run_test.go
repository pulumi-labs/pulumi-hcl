...
// TestEngine_SameLocalNameDifferentSourceCollapses documents a known limitation
// of the alias model (see https://github.com/pulumi-labs/pulumi-hcl/issues/184):
// at runtime, a resource is mapped to its provider purely by the local-name
// prefix of its type, with no module scoping and no consultation of the source
// declared in that module's required_providers. So two modules that alias
// *different* sources to the *same* local name and both instantiate resources
// collapse onto a single provider package.
//
// Here module "a" declares p = "foo/aphid" and module "b" declares p =
// "bar/bee", and both use a "p_thing" resource. OpenTofu would resolve these to
// two distinct providers (foo/aphid and bar/bee); pulumi-hcl resolves both to
// package "p" by prefix, so the "bee" package below is never reached. The
// install path is unaffected — GetRequiredPackages already emits both sources
// (see TestGetRequiredPackages_DistinctSourcesSameLocalName) — this gap is only
// in runtime resource-to-provider resolution.
func TestEngine_SameLocalNameDifferentSourceCollapses(t *testing.T) {
	t.Parallel()

	tmp := t.TempDir()
	require.NoError(t, os.WriteFile(tmp+"/main.tf", []byte(`
module "a" { source = "./a" }
module "b" { source = "./b" }
`), 0o644))

	require.NoError(t, os.MkdirAll(tmp+"/a", 0o755))
	require.NoError(t, os.WriteFile(tmp+"/a/main.tf", []byte(`
terraform {
  required_providers {
    p = { source = "foo/aphid" }
  }
...
`), 0o644))
	...
	require.NoError(t, os.MkdirAll(tmp+"/b", 0o755))
	require.NoError(t, os.WriteFile(tmp+"/b/main.tf", []byte(`
terraform {
  required_providers {
    p = { source = "bar/bee" }
  }
...
`), 0o644))
	...
	p := parser.NewParser()
	config, diags := p.ParseDirectory(tmp)
	require.False(t, diags.HasErrors(), "parse error: %s", diags.Error())

	thing := schema.ResourceSpec{}
	mock := &testutil.MockResourceMonitor{}
	engine := run.NewEngine(t.Context(), config, &run.EngineOptions{
		ProjectName:     "test-project",
		StackName:       "dev",
		...
		SchemaLoader: schemaloader.New(t,
			schema.PackageSpec{Name: "p", Resources: map[string]schema.ResourceSpec{"p:index:Thing": thing}},
			// b declares source "bar/bee"; if the source were honored its
			// resource would land here. It never does.
			schema.PackageSpec{Name: "bee", Resources: map[string]schema.ResourceSpec{"bee:index:Thing": thing}},
		),
	})
	...
	// Both modules' resources resolve to the same package, by local name "p" —
	// module b's declared "bar/bee" source is ignored and "bee" is never loaded.
	assert.Equal(t, "p:index:Thing", byName["a-x"])
	assert.Equal(t, "p:index:Thing", byName["b-y"])
}
