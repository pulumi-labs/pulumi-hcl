...
func runRemoteExec(ctx context.Context, spec *Spec, hclCtx *hcl.EvalContext) error {
	...
	switch {
	case len(inline) > 0:
		return runInline(comm, inline)
	case script != "":
		return runScripts(comm, []string{script})
	case len(scripts) > 0:
		return runScripts(comm, scripts)
	}
	return fmt.Errorf("remote-exec: no commands specified")
}

// TF joins inline lines into a single shell invocation.
func runInline(comm communicator.Communicator, inline []string) error {
	return execRemote(comm, strings.Join(inline, "\n"))
}

// TF leaves uploaded scripts on the remote; we match that.
func runScripts(comm communicator.Communicator, paths []string) error {
	for _, localPath := range paths {
		...
		if err := execRemote(comm, remotePath); err != nil {
...
func execRemote(comm communicator.Communicator, command string) error {
	cmd := &remote.Cmd{
		Command: command,
		Stdout:  os.Stdout,
		Stderr:  os.Stderr,
